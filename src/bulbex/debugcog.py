...
class LinkedGuildsEmbed(discord.Embed):
    ...
    def __init__(self, guilds: List[discord.Guild], *args, **kwargs):
        ...
        self.title = "Подключенные гильдии"
...
class DebugCog(commands.Cog):
    ...
    async def sync(self, ctx: discord.ApplicationContext):
        ...
        logger.info(
            f"{ctx.guild.name} | Вызов /debug_resync от {ctx.author.name} в чате {ctx.channel.name}."
        )

        await ctx.respond("**Запущена повторная синхронизация...**")
        await self._bot.sync_commands(
            guild_ids=[guild.id for guild in self._bot.guilds]
        )
    ...
    async def guilds(self, ctx: discord.ApplicationContext):
        ...
        logger.info(
            f"{ctx.guild.name} | Вызов /debug_guilds от {ctx.author.name} в чате {ctx.channel.name}."
        )
